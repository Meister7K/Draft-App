  ...
  if (!player?.player_info || !context) {
    return {
      ...
      factors: {
        ...
        playerValue: { score: 0, explanation: "Invalid player or context data" },
        competition: { score: 0, explanation: "Invalid player or context data" },
        availability: { score: 0, explanation: "Invalid player or context data" },
        startingLineupImpact: { score: 0, explanation: "Invalid player or context data" }
      }
    };
  }
...
export function calculateRosterNeedScore(player, context) {
  ...
  if (currentCount < coreNeeded) {
    ...
    score = Math.min(100, 60 + (remaining * 20)); // 60-100 based on remaining need
    explanation = `High need: ${remaining} more ${position}${remaining > 1 ? 's' : ''} needed for core lineup`;
    ...
    if (totalRbWrCount < totalPossibleSlots) {
      score = Math.max(30, 70 - (currentCount * 10)); // 30-70 based on position depth
      explanation = `Moderate need: Can fill FLEX slots, ${totalPossibleSlots - totalRbWrCount} total RB/WR/TE slots remaining`;
    } else {
      score = 15;
    ...
    score = Math.max(10, 40 - (currentCount * 10));
...
export function calculatePlayerValueScore(player, context) {
  ...
  try {
    ...
    const compositeValue = calculateCompositeValue(player, false, currentPickNumber);
    
    // Normalize composite value to 0-100 scale
    // Based on analysis of typical composite values (0-200+ range)
    const normalizedScore = Math.min(100, Math.max(0, compositeValue / 2));
...
export function calculateCompetitionScore(player, context) {
  ...
  if (context.leagueAnalysis && context.urgencyScores) {
    const { calculateEnhancedCompetitionScore } = require('./CompetitionAnalyzer.js');
...
export function calculateAvailabilityScore(player, context) {
  ...
  if (context.leagueAnalysis && context.targetingPrediction && context.draftOrder) {
    const { calculatePlayerAvailability } = require('./AvailabilityPredictor.js');
    ...
    try {
      ...
      // Convert availability percentage to score (higher availability = higher score)
...
export function calculateStartingLineupImpact(player, context) {
  ...
  try {
    ...
    const { calculateProjectedFantasyPointImprovement } = require('./StartingLineupImpactCalculator.js');
    
    const impactAnalysis = calculateProjectedFantasyPointImprovement(player, context);
    ...
    console.warn('Enhanced starting lineup impact calculation failed, using fallback:', error);
...
function calculateBasicStartingLineupImpact(player, context) {
  ...
  if (filledStarters.length > 0) {
    const worstStarter = filledStarters.reduce((worst, current) => {
      const currentPoints = current.player?.player_info?.projected_2025_points || 0;
      const worstPoints = worst.player?.player_info?.projected_2025_points || 0;
      return currentPoints < worstPoints ? current : worst;
    });
    worstStarterPoints = worstStarter.player?.player_info?.projected_2025_points || 0;
  ...
  if (["RB", "WR", "TE"].includes(position) && currentStarters.FLEX) {
    const flexStarters = currentStarters.FLEX.filter(starter => starter !== null);
    if (flexStarters.length > 0) {
      ...
      const worstFlexPoints = worstFlexStarter.player?.player_info?.projected_2025_points || 0;
  ...
  if (hasEmptySlots) {
    ...
    explanation = `Would fill empty starting slot with ${projectedPoints.toFixed(1)} projected points`;
    ...
    explanation = `Would improve starting lineup by ${pointsImprovement.toFixed(1)} points per week`;
  } else {
...
export function generateRankedRecommendations(availablePlayers, context) {
  if (!availablePlayers || !Array.isArray(availablePlayers) || availablePlayers.length === 0) {
    return [];
  }
  ...
  const scoredPlayers = availablePlayers
    .map(player => {
      ...
      return {
        player,
        optimization,
        playerId: player.player_info?.player_id || `${player.player_info?.name}-${player.player_info?.position}`
      };
    })
  ...
  const filteredRecommendations = filterTopRecommendationsWithDiversity(rankedPlayers, 5);
...
export function applyTieBreakingLogic(playerA, playerB) {
  ...
  const rosterNeedDiff = playerB.optimization.factors.rosterNeed.score - playerA.optimization.factors.rosterNeed.score;
  if (Math.abs(rosterNeedDiff) >= 5) {
    return rosterNeedDiff;
  }
  
  // Tie-breaker 2: Player value score (higher is better)
  const playerValueDiff = playerB.optimization.factors.playerValue.score - playerA.optimization.factors.playerValue.score;
 ...
 */
export function filterTopRecommendationsWithDiversity(rankedPlayers, maxRecommendations = 5) {
  ...
  if (recommendations.length < maxRecommendations) {
    const remainingPlayers = rankedPlayers.filter(player => 
...
export function generateRecommendationAction(recommendation, context) {
  ...
  // High availability, low roster need = WAIT
    ...
    action = "WAIT";
    reasoning = "Player likely available later, focus on more pressing needs first";
    riskAssessment = "Low risk - similar players should be available in later rounds";
  }
...
export function calculateRecommendationConfidence(optimization) {
  ...
  const avgFactorScore = factorScores.reduce((sum, score) => sum + score, 0) / factorScores.length;
  const factorVariance = factorScores.reduce((sum, score) => sum + Math.pow(score - avgFactorScore, 2), 0) / factorScores.length;
...
export function assessRosterNeeds(currentRoster, rosterFormat) {
  ...
  rosterFormat.forEach(({ position, slots }) => {
    ...
    positionNeeds[position] = {
      ...
      needed: needed,
      urgency: needed === 0 ? 'none' : (needed >= Math.ceil(slots * 0.5) ? 'high' : 'medium')
    };
  ...
  if (totalNeeds === 0) {
    ...
    const criticalPositions = criticalNeeds.map(need => 
      `${need.needed} ${need.position}${need.needed > 1 ? 's' : ''}`
    ).join(', ');
