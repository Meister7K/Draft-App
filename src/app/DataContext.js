...
export function DataProvider({ children }) {
  ...
  const setPlayerData = (player_id, playerDataObj) =>
    setData((d) => ({ ...d, playerData: { ...d.playerData, [player_id]: playerDataObj } }));
  const submitUserForm = (userFormData) => setData((d) => ({ ...d, userFormData }));
  const setLeagueUsers = (leagueUsers) => setData((d) => ({ ...d, leagueUsers })); // NEW
  const clearAllData = () => {
    setData(defaultState);
